  ...
  return (
    <>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map((item, index) => {
        ...
        return <article key={item.id} className={cn(panel, 'flex min-h-[270px] flex-col p-6', index === 0 && 'border-[var(--app-text)] bg-[var(--app-text)] text-[var(--app-bg)]')}><div className={cn('flex justify-between text-[9px] text-[var(--app-muted)]', index === 0 && 'text-[var(--app-muted)]')}><span className={cn('rounded-md bg-[var(--app-subtle)] px-2 py-1 font-bold text-[var(--app-text)]', index === 0 && 'bg-[var(--app-bg)]/10 text-[var(--app-bg)]')}>{item.tag}</span><span>{item.date}</span></div><h2 className="mt-7 text-xl font-bold leading-snug">{item.title}</h2><p className={cn('mt-3 text-xs leading-relaxed text-[var(--app-muted)]', index === 0 && 'text-[var(--app-muted)]')}>{item.summary}</p><div className={cn('mt-auto flex items-center gap-3 border-t border-[var(--app-border)] pt-5', index === 0 && 'border-white/10')}><span className="grid h-9 w-9 place-items-center rounded-xl text-[10px] font-extrabold text-white" style={{ background: company?.color ?? '#3a3a3a' }}>{item.symbol === 'ALL' ? 'PH' : item.symbol.slice(0, 2)}</span><span><strong className="block text-xs">{item.scope} briefing</strong><small className="text-[9px] text-[var(--app-muted)]">{company?.shortName ?? 'Industrial sector'}</small></span></div></article>
      })}</section>
    </>
  )
}
