...
export default function ValuationLab() {
  ...
  return (
    <>
      ...
      <section className="mt-11 grid gap-9 rounded-[21px] bg-[var(--app-text)] p-7 text-[var(--app-bg)] lg:grid-cols-[.85fr_1.15fr] lg:p-9">
        <div><p className="text-[10px] font-bold uppercase tracking-[.15em] text-[var(--app-bg)]">Philippine adjustment</p><h2 className="mt-2 text-2xl font-bold">Local rates replace the U.S. AAA yield.</h2><p className="mt-3 text-xs leading-relaxed text-[var(--app-muted)]">{PHILIPPINE_ASSUMPTIONS.note}</p><a className="mt-4 inline-flex items-center gap-2 text-[10px] font-bold text-[var(--app-bg)]" href={PHILIPPINE_ASSUMPTIONS.sourceUrl} target="_blank" rel="noreferrer">Official Treasury auction result <ExternalLink className="h-3 w-3" /></a></div>
...
function Assumption({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-white/[.09] bg-[var(--app-surface)]/[.04] p-3"><dt className="text-[9px] text-[var(--app-muted)]">{label}</dt><dd className="mt-1 font-serif text-lg">{value}</dd></div>
}
